...
def medu_netcher_render(signs, sign_map, title=None, font_size=48, font_family="Noto Sans Egyptian Hieroglyphs", as_svg=False, vertical=False):
    unicode_chars = []
    for code in signs:
    ...
    if as_svg:
        height = (len(unicode_chars) + 2) * font_size  # +2 for title and spacing
        dwg = svgwrite.Drawing(size=(font_size * 2, height))
        y = font_size
        if title:
            dwg.add(dwg.text(
                title,
...
def main():
    parser = argparse.ArgumentParser(description="Render Medu NeTcher signs as Unicode or SVG.")
